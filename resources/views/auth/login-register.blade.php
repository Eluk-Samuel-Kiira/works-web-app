...
<head>
    ...
    <!-- Your existing core CSS -->
    <link rel="stylesheet" href="{{ web_asset('front/css/styles.css') }}" />
    
    <style>
        /* Additional styles specific to auth page - matching your theme */
        .auth-wrapper {
            min-height: 100vh;
            display: flex;
            align-items: center;
        ...
        .auth-tab.active {
            ...
            color: #fff;
            box-shadow: 0 2px 8px rgba(93,135,255,.25);
        }
        ...
        .role-btn span:first-child {
            font-size: 24px;
            display: block;
            margin-bottom: 6px;
        }
        
        .role-btn span:nth-child(2) {
            font-size: 13px;
            font-weight: 600;
            color: #1e293b;
            display: block;
...
<body style="background: #f8f9fc;">
    ...
    <a href="/" class="logo-link">
        <img src="{{ getFavicon() }}" alt="Stardena" style="width: 22px; height: auto;">
        ...
        <div class="auth-card">
            ...
            <div id="loginForm">
                <div style="margin-bottom: 20px;">
                    <span style="display:inline-flex;align-items:center;gap:5px;background:#eef2ff;color:#4f46e5;font-size:11px;font-weight:600;padding:4px 12px;border-radius:20px;margin-bottom:14px;">
                        ✦ No password needed
                    </span>
                ...
                <button id="loginBtn" onclick="sendMagicLink()" class="btn-submit">
                    Send magic link ✉
                </button>
                
                <p style="text-align:center;font-size:13px;color:#64748b;margin-top:16px;">
                    No account?
                    <a href="javascript:void(0)" onclick="switchTab('register')" style="color:var(--bs-primary);font-weight:600;text-decoration:none;">
                        Create one free
                    </a>
                </p>
            </div>
            ...
            <div id="registerForm" style="display: none;">
                ...
                <div class="form-group">
                    ...
                    <input type="tel" id="phoneNumber" class="form-control" placeholder="+256 700 000 000">
                </div>
                ...
                <button id="registerBtn" onclick="registerUser()" class="btn-submit">
                    Create account & get magic link ✉
                </button>
                
                <p style="text-align:center;font-size:13px;color:#64748b;margin-top:16px;">
                    Already have an account?
                    <a href="javascript:void(0)" onclick="switchTab('login')" style="color:var(--bs-primary);font-weight:600;text-decoration:none;">
                        Log in
                    </a>
                </p>
            </div>
            ...
            <div id="successView" style="display:none;text-align:center;padding:1.5rem 0;">
                ...
                <p id="successMessage" style="font-size:13px;color:#64748b;margin:0 0 20px;line-height:1.6;"></p>
                <p style="font-size:13px;color:#64748b;">
                    Didn't receive it?
                    <a href="javascript:void(0)" onclick="resendLink()" style="color:var(--bs-primary);font-weight:600;text-decoration:none;">
                        Resend link
                    </a>
                </p>
    ...
    <script>
        // API Configuration - Same as your modal
        ...
        function showAlert(message, type = 'error') {
            ...
            container.innerHTML = `<div class="alert-message alert-${type}"><span>${message}</span></div>`;
        ...
        // Send magic link - Same as modal API call
        async function sendMagicLink() {
            ...
            try {
                ...
                if (response.ok && data.success) {
                    ...
                    showAlert(data.message || 'No account found. Please register first.', 'error');
        ...
        // Register user - Same as modal API call
        async function registerUser() {
            ...
            if (!email || !isValidEmail(email)) {
                showAlert('A valid email address is required.', 'error');
                return;
            }
            if (!terms) {
            ...
            try {
                ...
                if (response.ok && data.success) {
                    ...
                    showAlert(data.message || 'Unable to create account. Please try again.', 'error');
                    showLoading('registerBtn', false);
                }
            } catch (error) {
                console.error('Error:', error);
        ...
        async function resendLink() {
            ...
            try {
                ...
                if (response.ok) {
                    showAlert('Link resent — check your inbox again.', 'success');
                } else {
                    showAlert('Failed to resend link. Please try again.', 'error');
                }
            } catch (error) {
                console.error('Error:', error);
                showAlert('Connection error. Please try again.', 'error');
